...
const cacheFile = 'dist/generator/data/cache.json';
const generationsFiles = 'dist/src/data/generations/';

const isCacheAvailable = fs.existsSync(cacheFile);
let cache: Record<string, string> = {};

if (isCacheAvailable) {
  const rawData = fs.readFileSync(cacheFile).toString();
...
for (const key in projectPokemonPages) {
  ...
  fs.writeFileSync(`${generationsFiles}${key}.json`, JSON.stringify(images, null, 2));
}
